...
export const EditProfilePage = ({ onNavigate }) => {
  ...
  const handlePdfUpload = async (e) => {
    ...
    try {
      ...
      console.error('Error uploading biodata to Firebase Storage:', err);
      alert('Failed to upload file to Firebase Storage.');
    }
  };
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="bg-white rounded-3xl border border-brand-rose/20 shadow-2xl p-6 sm:p-10 space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-xs font-semibold text-brand-plum mb-1">
              Partner Expectations (अपेक्षा - भागीदाराकडून अपेक्षा)
            </label>
            <textarea
              rows={4}
              value={formData.expectations}
              onChange={(e) => setFormData({ ...formData, expectations: e.target.value })}
              placeholder="अपेक्षा सांगा: Write your partner expectations (e.g. Education, Profession, Family background, Location, Height, Diet...)"
              className="w-full p-3 rounded-xl border border-brand-plum/30 bg-amber-50/20 text-xs focus:ring-2 focus:ring-brand-plum/20"
            />
          </div>
