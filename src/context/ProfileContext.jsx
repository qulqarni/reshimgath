...
import { 
  ...
  saveSubscriptionPlansToFirestore,
...
export const ProfileProvider = ({ children }) => {
  ...
  const [subscriptionPlans, setSubscriptionPlans] = useState(() => {
    const saved = localStorage.getItem('reshimgath_subscription_plans');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
  ...
  const { user, hasActiveSubscription, canViewProfile, unlockProfileForUser } = useAuth();
  ...
  useEffect(() => {
    ...
    const loadOtherContent = async () => {
      ...
      const firestorePlans = await fetchSubscriptionPlansFromFirestore();
      if (firestorePlans && Array.isArray(firestorePlans) && firestorePlans.length > 0) {
        setSubscriptionPlans(firestorePlans);
        try {
          localStorage.setItem('reshimgath_subscription_plans', JSON.stringify(firestorePlans));
        } catch (e) {}
      }
    };
    loadOtherContent();
  ...
  useEffect(() => {
    try {
      localStorage.setItem('reshimgath_subscription_plans', JSON.stringify(subscriptionPlans));
    } catch (e) {}
  }, [subscriptionPlans]);
  ...
  const updateSubscriptionPlans = async (newPlans) => {
    setSubscriptionPlans(newPlans);
    try {
      localStorage.setItem('reshimgath_subscription_plans', JSON.stringify(newPlans));
    } catch (e) {}
    await saveSubscriptionPlansToFirestore(newPlans);
    addToast('Subscription plan pricing updated successfully!', 'success');
  };
